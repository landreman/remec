...
def test_closed_field_finite_anisotropy_axis(perpendicular_conductivity: float) -> None:
    ...
    exact_amplitude = 1.0 / (2.0 * ng.pi**2 * perpendicular_conductivity)
    exact = exact_amplitude * flux
    central_amplitude = float(solution._field(solution._mesh(0.5, 0.5)))
    relative_l2_error = float(
        ng.sqrt(ng.Integrate((solution._field - exact) ** 2, solution._mesh, order=10))
        / ng.sqrt(ng.Integrate(exact**2, solution._mesh, order=10))
    )
    ...
    assert central_amplitude == pytest.approx(exact_amplitude, rel=0.02)
    assert relative_l2_error < 0.02
    assert tangency_l2_squared < 1.0e-12
    assert solution.free_dof_relative_residual_norm < 1.0e-10
...
def test_closed_field_full_order_resolution_anisotropy_scan() -> None:
    ...
    for row in rows:
        ...
        central_amplitude = float(solution._field(solution._mesh(0.5, 0.5)))
        effective_perpendicular_diffusivity = 1.0 / (2.0 * ng.pi**2 * central_amplitude)
...
def test_analytic_island_manufactured_convergence(polynomial_order: int) -> None:
    ...
    for maxh in mesh_sizes:
        ...
        l2_error_squared = ng.Integrate((solution._field - exact) ** 2, solution._mesh, order=10)
        ...
        assert solution.field_direction_diagnostics.floor_activity_l2_squared > 0.0
        assert solution.free_dof_relative_residual_norm < 1.0e-10
