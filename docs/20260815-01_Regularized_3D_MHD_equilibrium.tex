...
width. Because this flux also relaxes the mean parallel current, which classical theory leaves free
and a code must accept as input, the closure is constrained: the flux acts on the departure of
$J_\parallel/B$ from an unknown one-dimensional profile that is fixed by prescribing the enclosed
toroidal current versus normalized enclosed volume $s=V/V_\Omega\in[0,1]$---the same independent
variable as the pressure input. The
result is a closed elliptic/hypoelliptic system of partial differential equations for
$(\bB,p,u=J_\parallel/B)$, bordered by one one-dimensional unknown and one one-dimensional
constraint, that is suitable as the basis of a practical three-dimensional equilibrium code, reduces to the classical model (and to Grad--Shafranov plus one-dimensional transport) when
...
one-dimensional pressure profile $p_0(s)$, specified as a function of normalized enclosed volume,
is realized exactly
...
$\propto D_u^{1/3}$ and excludes $\delta$-function sheets. The same transverse diffusion also
removes the freedom to prescribe the mean parallel current, and Sec.~\ref{sec:net_current} shows
that---contrary to a natural first guess---no change of variables restores it: the flux must
instead act on the departure of $u$ from an unknown one-dimensional profile $G(s)$, with $G$ fixed
by an enclosed-toroidal-current constraint $I_{\rm tor}(s)=I_0(s)$ on the level sets of the
normalized-volume label $s$. Section~\ref{sec:model} assembles the
...
principal interest here---in which a one-dimensional pressure-versus-normalized-volume profile
$p_0(s)$ is
...
algorithm and verification tests; Sec.~\ref{sec:braginskii} gives the Braginskii reduction and
explicit transport coefficients; Sec.~\ref{sec:relation} relates the
...
prescribed $p_0(s)$ exactly while the transport operator retains control of \emph{where} the
...
A production code could instead evolve $n$, $T_e$, $T_i$ separately with particle sources and
inter-species exchange; Sec.~\ref{sec:braginskii} gives the underlying two-fluid reduction and
coefficients, and nothing below depends on this simplification.
...
\subsection{The mean parallel current: a constrained regularization}
\label{sec:net_current}

One genuine freedom of the classical problem must be handed back to the user, and it turns out
that doing so requires an additional equation, not merely a change of variables. On nested
surfaces at $D_u=0$, the flux-surface-averaged part of $u$ is undetermined by the
MDE~(\ref{eq:MDE}): $u\rightarrow u+G(\psi)$ maps solutions to solutions for any flux function
$G$. This nullspace is the net parallel current profile (ohmic, bootstrap, driven), a physical
input in every classical code, and the equation that fixes it in reality---the surface-averaged
parallel Ohm's law, with loop voltage and bootstrap drive---is not part of the present model.
The elliptic term in Eq.~(\ref{eq:u_equation}) removes the nullspace: it diffusively relaxes the
mean of $u$ across level sets toward a state selected only by the boundary value and by the
regularization itself (a Taylor-like state \cite{boozer1986}), appropriate for a fully relaxed
plasma but not for a driven one.

\paragraph{A change of variables does not restore the freedom.}
It is tempting to write $u=F(p)+\tilde u$ with a prescribed profile $F$, to note that
$\bB\cdot\nabla F(p)=F'(p)\,\bB\cdot\nabla p=O(\varepsilon_\kappa)$ so that $F(p)$ is an
approximate homogeneous solution of the unregularized MDE, and to solve
Eq.~(\ref{eq:u_equation}) for $\tilde u$ with $\tilde u=0$ on $\partial\Omega$, moving the $F$
terms to the right-hand side. This does nothing. For fixed $\bB$ and $p$,
Eq.~(\ref{eq:u_equation}) with $D_u>0$ is a linear hypoelliptic problem with a Dirichlet
condition and a unique solution; the equation for $\tilde u$ is Eq.~(\ref{eq:u_equation}) with
$u=F(p)+\tilde u$ substituted, and the boundary condition $\tilde u=0$ is $u=F(p_b)$. Two
profiles $F_1,F_2$ with $F_1(p_b)=F_2(p_b)$ therefore produce identically the same $u$, with
$\tilde u_2-\tilde u_1=F_1(p)-F_2(p)$: the interior shape of $F$ has no effect on the physical
solution, and only the single number $F(p_b)$ enters, through the boundary condition. The
seemingly small term $\nabla\cdot(D_uF'\gradperp p)$ that the substitution generates is indeed
$O(D_u)$, but the operator governing the level-set mean of $\tilde u$ is the transverse
diffusion, itself $O(D_u)$ (the streaming term $\bB\cdot\nabla$ has zero mean), so the response
of $\langle\tilde u\rangle$ to that forcing is $O(1)$ and cancels $F$ exactly. This is the
elementary singular-perturbation fact that $-D_u\,d^2U/d\psi^2=0$ determines $U$ completely for
every $D_u>0$, however small; the coefficient cancels from the equation for the formerly free
mode. (An earlier version of this note asserted the contrary; the assertion was wrong.)

The point can be made exactly, without averaging approximations, and the resulting identity is
the tool used below. Let $s(\br)$ be any smooth global label whose level sets are the isobars,
$\nabla p\parallel\nabla s$ [in interpretive mode $s=V_\chi(\chi)/V_\Omega$, Eq.~(\ref{eq:s_label})
below], and let $\Omega_s=\{\br:s(\br)<s\}$. Integrating $\nabla\cdot\bJ=0$ over $\Omega_s$
with the current~(\ref{eq:J_decomposition})--(\ref{eq:Jpi}), and using that the diamagnetic
current is exactly tangent to the isobars, gives on every level set
\begin{equation}
\oint_{s(\br)=s}D_u\,\gradperp u\cdot\nabla s\;\frac{dA}{|\nabla s|}
=\oint_{s(\br)=s}u\,\bB\cdot\nabla s\;\frac{dA}{|\nabla s|} .
\label{eq:levelset_current_identity}
\end{equation}
Writing $u=\bar u(s)+u_{\rm fl}$ with $\bar u$ the level-set average defined in
Eq.~(\ref{eq:levelset_average}) below, and $\gradperp u\cdot\nabla s=\bar u'(s)|\gradperp
s|^{2}+\gradperp u_{\rm fl}\cdot\nabla s$, Eq.~(\ref{eq:levelset_current_identity}) is a
first-order ordinary differential equation in $s$ for $\bar u$, with the edge value fixed by the
boundary condition: the mean current is set entirely by the physical parallel current crossing
the isobars [the right-hand side, $O(\varepsilon_\kappa)$, whose ratio to the left is
$O(\varepsilon_\kappa/\epsilon_J)$---a ratio of two small parameters, not automatically small]
and by the correlation of the fluctuating Pfirsch--Schl\"uter current with the level-set metric.
No prescribed profile appears anywhere. The regularization has removed the unwanted singular
nullspace and the wanted physical one together, and the system must be supplemented by one
one-dimensional constraint and one one-dimensional unknown.

\paragraph{The constrained closure.}
Let
\begin{equation}
s(\br)\equiv\frac{V_\chi\big(\chi(\br)\big)}{V_\Omega}\in[0,1]
\label{eq:s_label}
\end{equation}
be the normalized-volume label of the isobars (interpretive mode; in predictive mode the analogous
label built on the level sets of $p$, or a reference potential solved with $S_{\rm ref}$ for the
purpose), so that $\Omega_s$ has volume $sV_\Omega$, $s=0$ at the pressure maximum and $s=1$ on
$\partial\Omega$. We prefer $s$ to $p$ as the independent variable because it remains a
well-behaved coordinate across the edge plateau and the flattened islands where $p_0'$ vanishes,
and because it makes the pressure and current inputs parallel: $p_0(s)$ and $I_0(s)$ below are
both functions of the same normalized enclosed volume. Split
\begin{equation}
u=G(s)+\tilde u ,
\label{eq:u_split}
\end{equation}
where $G$ is now an \emph{unknown} one-dimensional function, and let the current-viscosity flux act
on the departure from it,
\begin{equation}
\bJ_\pi=-\,D_u\,\gradperp\tilde u=-\,D_u\big[\gradperp u-G'(s)\,\gradperp s\big] .
\label{eq:Jpi_constrained}
\end{equation}
Both modifications are needed: if the flux acted on the full $u$, $G$ would not enter the equations
at all; if $G$ were prescribed rather than solved for, its mean would be polluted by the terms of
Eq.~(\ref{eq:levelset_current_identity}). With Eq.~(\ref{eq:Jpi_constrained}), $\nabla\cdot\bJ=0$
written out is
\begin{equation}
\bB\cdot\nabla u-\nabla\cdot\big(D_u\gradperp\tilde u\big)
=\frac{2}{B^{3}}\,\bB\cdot\big(\nabla p\times\nabla B\big)
-\frac{\mu_0 u}{B^{2}}\,\bB\cdot\nabla p
+\frac{\mu_0 D_u}{B^{2}}\,\gradperp\tilde u\cdot\nabla p ,
\label{eq:u_equation_constrained}
\end{equation}
or, in the variable $\tilde u$,
\begin{equation}
\bB\cdot\nabla\tilde u-\nabla\cdot\big(D_u\gradperp\tilde u\big)
=\frac{2}{B^{3}}\,\bB\cdot\big(\nabla p\times\nabla B\big)
-\frac{\mu_0 u}{B^{2}}\,\bB\cdot\nabla p
...
\end{equation}
with $\tilde u=0$ on $\partial\Omega$ and $G(1)=u_b$, the prescribed edge value of $u$ ($u_b=0$
for a wall in vacuum, Sec.~\ref{sec:edge}). Exactness of $\nabla\cdot\bJ=0$ is preserved because
the multiplier lives in the constitutive current and the last term of
Eq.~(\ref{eq:u_equation_constrained}) uses the matching gradient; a source added directly to
Eq.~(\ref{eq:u_equation}) would instead destroy the integrability of Amp\`ere's law. The layer
analysis of Sec.~\ref{sec:layer} is unchanged, since $G(s)$ is smooth and enters only through the
$O(\varepsilon_\kappa)$ forcing $G'\bB\cdot\nabla s$; the quadratic form
$\int D_u|\gradperp\tilde u|^{2}\ge0$ is retained; and $\bJ_\pi$ is $O(D_u)$ for bounded $G'$.
We call Eq.~(\ref{eq:Jpi_constrained}) a constrained regularization closure. It has a natural
physical reading---in the mean-field Ohm's law from which the current viscosity is inherited
\cite{boozer1986}, hyper-resistivity smooths $J_\parallel$ toward the profile driven by loop
voltage and bootstrap current, not toward zero, and $G$ stands in for that driven profile---but no
stronger thermodynamic interpretation is claimed: $G$ is the one-dimensional datum that the
missing Ohm's law would supply, imported here as an input.

The unknown $G$ is fixed by a one-dimensional constraint. Define the level-set (thin-shell,
volume-weighted) average, which for the normalized-volume label has the particularly simple form
\begin{equation}
\langle f\rangle_s\equiv
\frac{\oint_{s(\br)=s}f\,dA/|\nabla s|}{\oint_{s(\br)=s}dA/|\nabla s|}
=\frac{1}{V_\Omega}\frac{d}{ds}\int_{\Omega_s}f\,d^{3}r ,
\label{eq:levelset_average}
\end{equation}
by the co-area formula and $dV/ds=V_\Omega$; the second form is the layer-cake integral of
Sec.~\ref{sec:moments}, is evaluated by the same machinery as $V_\chi$, and remains meaningful when
a level set has several components. The natural constrained quantity is the enclosed toroidal
current. Because $\bJ$ crosses the level sets at $O(\varepsilon_\kappa,\epsilon_J)$, the current
through a poloidal cross-section $\Sigma_\phi(s)=\{\phi=\mathrm{const}\}\cap\Omega_s$ depends
weakly on the toroidal angle at which it is taken; the arbitrariness is removed by the toroidal
average, which has the volume representation
\begin{equation}
I_{\rm tor}(s)\equiv\frac{1}{2\pi}\int_0^{2\pi}\!d\phi\int_{\Sigma_\phi(s)}\bJ\cdot\bn_\phi\,dA
=\frac{1}{2\pi}\int_{\Omega_s}\bJ\cdot\nabla\phi\;d^{3}r ,
\qquad
\frac{dI_{\rm tor}}{ds}=\frac{V_\Omega}{2\pi}\,\langle\bJ\cdot\nabla\phi\rangle_s ,
\label{eq:Itor}
\end{equation}
($d^3r=R\,dR\,dZ\,d\phi$ and $\bn_\phi=R\nabla\phi$), which is smoother and cheaper to evaluate
than many explicit poloidal cuts. The user prescribes
\begin{equation}
I_{\rm tor}(s)=I_0(s),\qquad 0<s\le1,\qquad I_0(0)=0 ,
\label{eq:current_constraint}
\end{equation}
with $I_0\equiv0$ for a net-current-free stellarator, or $I_0$ from an ohmic-plus-bootstrap
model iterated to consistency with the equilibrium. This is what most equilibrium codes take as
their current input, and on nested surfaces it reduces exactly to the enclosed toroidal plasma
current versus volume, i.e.\ versus $\psi$ through $V(\psi)$. Should $\langle u\rangle_s$ or
$\langle\bJ\cdot\bB\rangle_s$ be preferred as the input (for instance to accept a neoclassical
$\langle J_\parallel B\rangle$ directly), the constraint has the same structure with a different
weight under the average, and a code can offer the alternatives behind a switch; we note only that
$u$ is not the toroidal current density---the diamagnetic and viscous currents also carry
toroidal current---so translating between the two inputs involves geometry-dependent corrections
that Eq.~(\ref{eq:current_constraint}) sidesteps.

\paragraph{Why the constrained problem is well posed.}
Perturb $G$ by $\delta G(s)$ with $\delta G(1)=0$. The particular solution $\delta u=\delta G(s)$
satisfies Eq.~(\ref{eq:u_equation_constrained}) up to the residual $\delta G'\,\bB\cdot\nabla s
=O(\varepsilon_\kappa)$, so $\delta u\simeq\delta G(s)$ and, from Eq.~(\ref{eq:Itor}),
\begin{equation}
\delta\Big(\frac{dI_{\rm tor}}{ds}\Big)
\simeq\frac{V_\Omega}{2\pi}\,\langle\bB\cdot\nabla\phi\rangle_s\,\delta G(s)
+O(\epsilon_J,\varepsilon_\kappa/\epsilon_J)\,\delta G ,
\label{eq:G_response}
\end{equation}
where $\langle\bB\cdot\nabla\phi\rangle_s$ is positive and $O(B/R)$ throughout a torus. The map
from the multiplier to the constraint is thus near-diagonal with a positive, $O(1)$ weight, and the
constrained linear problem for $(\tilde u,G)$ is well posed and well conditioned. Substituting
$u=G+\tilde u$ into Eq.~(\ref{eq:levelset_current_identity})---which now holds with $\tilde u$ in
place of $u$ on its left---shows that $\langle\tilde u\rangle_s$ is not zero but bounded, of
relative order $\max(\varepsilon_\kappa/\epsilon_J,\;\varepsilon_{\rm geom})$, and that $G$
absorbs it: $G$ equals the classical mean-current profile up to these small corrections, $G'$ is
bounded, and the multiplier current $D_uG'\gradperp s$ vanishes with $D_u$. This last property is
the essential check that the multiplier only selects the otherwise-undetermined one-dimensional
mode and does not become a finite artificial current in the physical limit.

\paragraph{Discrete form.}
The continuum count is one function of one variable against one constraint of one variable, and
the discretization should keep the count square. Choose nodes $0=s_0<s_1<\dots<s_N=1$---uniform
in $s$, i.e.\ equal-volume shells, or the knot sequence of the spline representing $p_0(s)$---and
represent $G$ on them by a piecewise-linear or cubic-B-spline basis with $G(s_N)=u_b$, leaving $N$
free coefficients. Impose the $N$ shell constraints
\begin{equation}
\frac{1}{2\pi}\int_{s_{j-1}<s(\br)<s_j}\bJ\cdot\nabla\phi\;d^{3}r
=I_0(s_j)-I_0(s_{j-1}),\qquad j=1,\dots,N ,
\label{eq:shell_constraints}
\end{equation}
which carry the same information as the cumulative profile [since $I_{\rm tor}(0)=0$] but are
local in $s$ and are exactly the smooth-integrand layer-cake moments of Sec.~\ref{sec:moments},
evaluated with the mollified Heaviside or sub-cell quadrature of Sec.~\ref{sec:Vchi}; $s=0$ is not
a constraint node, and $s=1$ fixes the total current. Within the Picard cycle of
Sec.~\ref{sec:numerics}, with $\bB$, $p$, $s$ frozen, the discretized Eq.~(\ref{eq:utilde_equation})
and Eq.~(\ref{eq:shell_constraints}) form the bordered linear system
\begin{equation}
...
\label{eq:bordered}
\end{equation}
where $A$ is the advection--diffusion operator already needed for (M3), acting on $\tilde u$
(streaming, diffusion, and the $\tilde u$ parts of the reaction and $D_u$ right-hand-side terms);
$P$ collects the action of the $N$ free basis functions of $G$, namely the forcing
$-G'\,\bB\cdot\nabla s$ \emph{and} the reaction contribution $-(\mu_0/B^{2})\,G\,\bB\cdot\nabla p$
from $u=G+\tilde u$, both $O(\varepsilon_\kappa)$ (the fixed $G(s_N)=u_b$ contribution moves to
the right-hand side); and $C_u$, $C_G$ are the shell integrals of the $\tilde u$ and $G$
contributions to $\bJ\cdot\nabla\phi$---in the variables $(\tilde u,\mathbf g)$ these are
$\tilde u\,\bB\cdot\nabla\phi-D_u\gradperp\tilde u\cdot\nabla\phi$ and $G\,\bB\cdot\nabla\phi$
respectively, the multiplier current $D_uG'\gradperp s$ being already contained in
$-D_u\gradperp\tilde u$ (it must not be added to $C_G$ as well), while the known diamagnetic
contribution moves to the right-hand side of the constraint rows. Since $N$ is tens while $\tilde{\mathbf u}$ has millions of
degrees of freedom, the border is handled by a Schur complement with the existing $A$-solver as
preconditioner; in the Newton formulation of Sec.~\ref{sec:newton} the $N$ coefficients simply
join the state vector and Eq.~(\ref{eq:shell_constraints}) the residual. A fixed-point update
$G\leftarrow G+\big[I_0'-dI_{\rm tor}/ds\big]\big/\big[(V_\Omega/2\pi)\langle\bB\cdot\nabla\phi\rangle_s\big]$,
justified by Eq.~(\ref{eq:G_response}), is an adequate prototype and typically converges in a few
sweeps, but the neglected terms in Eq.~(\ref{eq:G_response}) need not be small when
$\varepsilon_\kappa$ and $\epsilon_J$ are poorly separated, so the coupled solve is the production
choice. As for $N$: each shell should span at least three or four radial grid cells so that its
volume integral is resolved and $G\circ s$ is representable on the mesh, and there is nothing to
gain from resolving $I_0'$ below the widths $\delta\propto D_u^{1/3}$ and $w_c$ on which the
model's own structure is smooth; $N=16$--$64$ for smooth targets, with the standard check that
results do not move when $N$ is doubled. If a user supplies $I_0$ with more structure than $G$ can
represent, the constraint is imposed in least squares with a mild smoothness penalty on $G$.

Three diagnostics should be reported with every run: $I_{\rm tor}(s)-I_0(s)$, zero to solver
tolerance; the toroidal variation $\Delta I_{\rm tor}(s)={\rm rms}_\phi[I_{\rm tor}(s,\phi)
-I_{\rm tor}(s)]$, which measures the leakage of current across isobars and must vanish as
$\varepsilon_\kappa,\epsilon_J\rightarrow0$; and $\langle\tilde u\rangle_s$ together with the
multiplier current $D_uG'\gradperp s$, which must vanish as $D_u\rightarrow0$ for an admissible
target. A simple unit test exposes the issue this subsection corrects and verifies its repair: hold
$\bB$, $p$, $D_u$ fixed and solve for the current with two very different profiles. In the
unconstrained formulation with $u=F(p)+\tilde u$ the two reconstructed $u$ fields agree to solver
tolerance; in the constrained formulation the two runs return their respective $I_0(s)$, while
their Pfirsch--Schl\"uter components converge to a common answer as $D_u$ is reduced. Thus $D_u$
regularizes only the otherwise singular three-dimensional current structure, and the
one-dimensional current profile is a genuine user input rather than an accidental output of the
regularizer---exactly as $\langle J_\parallel B\rangle(\psi)$ is in a classical code, but defined
without reference to surfaces.

\subsection{An isotropic variant of the current-viscosity closure}
\label{sec:isotropic_variant}

The constitutive relation~(\ref{eq:Jpi}) [in its constrained form~(\ref{eq:Jpi_constrained})]
regularizes current continuity with a purely transverse flux, $\bJ_\pi=-D_u\gradperp\tilde u$.
A natural variant replaces the perpendicular gradient by the full gradient,
\begin{equation}
\bJ_\pi=-\,D_u\,\nabla\tilde u ,\qquad \tilde u=u-G(s),
\label{eq:Jpi_iso}
\end{equation}
which adds a parallel component $-D_u\,\bb\,(\bb\cdot\nabla\tilde u)$ to the constitutive
current. (For brevity the remainder of this subsection is written for the unconstrained
variable $u$; the constrained closure carries over verbatim with $u\rightarrow\tilde u$ in every
$D_u$ term, and the enclosed-current constraint~(\ref{eq:current_constraint}) is unchanged.) Because a parallel current exerts no force, the perpendicular force
balance~(\ref{eq:perp_model}) and the diamagnetic current~(\ref{eq:Jperp}) are exactly
unchanged; the modification enters only the continuity closure. Two consequences must be
tracked. First, the decomposition~(\ref{eq:J_decomposition}) no longer has
$\bJ_\pi\perp\bB$, so the solved field $u$ and the physical $J_\parallel/B$ differ at
$O(D_u)$: $\bJ\cdot\bB/B^{2}=u-(D_u/B)\,\bb\cdot\nabla\tilde u$. One simply treats $u$ as the
auxiliary unknown and reconstructs $\bJ$ from~(\ref{eq:J_decomposition})
with~(\ref{eq:Jpi_iso}); diagnostics, the enclosed-current constraint~(\ref{eq:current_constraint}),
and any closure consuming $J_\parallel$ (e.g.\ a bootstrap model supplying $I_0(s)$ in
Sec.~\ref{sec:net_current}), must use the reconstructed current. Second, exactness of $\nabla\cdot\bJ=0$ requires the correction term of
Eq.~(\ref{eq:u_equation}) to use the \emph{matching} gradient: the isotropic variant of
the regularized continuity equation is
\begin{equation}
\bB\cdot\nabla u-\nabla\cdot\big(D_u\,\nabla u\big)
=\frac{2}{B^{3}}\,\bB\cdot\big(\nabla p\times\nabla B\big)
-\frac{\mu_0\,u}{B^{2}}\,\bB\cdot\nabla p
+\frac{\mu_0\,D_u}{B^{2}}\,\nabla u\cdot\nabla p ,
\label{eq:u_equation_iso}
\end{equation}
and with this pairing the integrability argument for Amp\`ere's law in
Sec.~\ref{sec:model} carries over verbatim, while a mixed pairing (transverse flux with
isotropic diffusion, or vice versa) leaves an $O(D_u)$ residual in $\nabla\cdot\bJ$ and
must be avoided.

The two closures are equivalent to the order the model is claimed accurate. The key
structural asymmetry with the pressure equation~(\ref{eq:pressure_equation}) is that in
Eq.~(\ref{eq:u_equation}) parallel transport is \emph{advective with an $O(1)$
coefficient}: at any parallel wavenumber $k_\parallel$ present in the solution, the added
parallel diffusion $D_u k_\parallel^{2}$ is smaller than the advection $k_\parallel B$ by
$D_u k_\parallel/B=\epsilon_J\,k_\parallel\bar L\ll1$. In the resonant layers of
Sec.~\ref{sec:layer} the added term contributes $+D_u k_\parallel^{2}\hat u$ to
Eq.~(\ref{eq:layer_equation}); at the layer edge, where
$k_\parallel(\delta)B\sim D_u/\delta^{2}$ defines the width, this is subdominant to the
retained transverse diffusion by the same small factor, and it vanishes at the layer
center where $k_\parallel\rightarrow0$, so the width and amplitude
scalings~(\ref{eq:layer_width}) are unchanged. In the constrained closure of
Sec.~\ref{sec:net_current} the only additional term is the $O(\varepsilon_\kappa)$ forcing
$-G'\bB\cdot\nabla s$ of Eq.~(\ref{eq:utilde_equation}), which is the same in both variants; the
response estimate~(\ref{eq:G_response}) is unaffected. The regularizing quadratic form remains
sign-definite, $\int D_u|\nabla\tilde u|^{2}\ge\int D_u|\gradperp\tilde u|^{2}\ge0$. On a boundary
with $\bB\cdot\bn=0$ the co-normal diffusive fluxes coincide,
$\bn\cdot(\bI-\bb\bb)\cdot\nabla u=\bn\cdot\nabla u$, so the boundary-condition count and
even the boundary-layer structure of the flux are identical. One property actually
improves: the operator in Eq.~(\ref{eq:u_equation_iso}) is \emph{uniformly elliptic}
independent of the geometry of $\bb$, so smoothness of $u$---and with it the exclusion of
current sheets---follows from standard elliptic regularity rather than the H\"ormander
bracket argument invoked after Eq.~(\ref{eq:u_equation}), and remains valid even where
$\bb$ is nearly degenerate (as can occur transiently for the iterates of a numerical
scheme, where $\gradperp=(\bI-\bb\bb)\cdot\nabla$ is ill-conditioned). The cost is one of
derivational fidelity: the kinetic argument of Sec.~\ref{sec:current} fixes the closure
to be \emph{transverse}---a parallel diffusion of $u$ corresponds physically to parallel
electron viscosity, whose coefficient is far larger than $D_u$, not equal to it---so
Eq.~(\ref{eq:Jpi_iso}) is an admissible \emph{numerical} variant, accurate to the order
of the retained physics, rather than an alternative physical model.

Numerically the isotropic variant has real attractions. For constant $D_u$ the
regularizing operator is a fixed, isotropic, symmetric-positive-definite Laplacian,
independent of the evolving $\bB$: it need not be reassembled as the field updates
within a nonlinear iteration, admits standard monotone discretizations and
preconditioners without the mixed-derivative stencils of the tensor operator, requires
no differentiation of $\bb\bb$ inside the divergence, contributes no
$\partial(\mathrm{operator})/\partial\bB$ block to a Newton linearization, and damps
grid-scale oscillations of $u$ \emph{along} field lines, which the transverse operator
leaves untouched. Conversely, discretizing $D_u\gradperp$ on a mesh not aligned with the
field incurs spurious parallel diffusion of magnitude at most $\sim D_u$ times the
misalignment error \cite{gunter2005,sharma2007}; the isotropic variant simply accepts,
by construction, the worst case of an error that the estimates above show to be benign.
We emphasize that this reasoning must \emph{never} be transplanted to the pressure
equation: there both directions are diffusive and the anisotropy ratio
$\varepsilon_\kappa$ \emph{is} the physics, so an isotropic contamination at the
$\kappa_\parallel$ level would destroy the critical width~(\ref{eq:wc}), the island
flattening, and the barrier structure entirely. A code should implement both closures
behind a runtime switch, applied consistently to the constitutive
current~(\ref{eq:J_decomposition}) and to every $D_u$ term of the continuity equation;
verify that the two solutions agree to $O(\epsilon_J)$ at fixed $D_u$ and converge to a
common answer in the $D_u\rightarrow0$ scan of Sec.~\ref{sec:numerics}---a cheap and
rather sharp verification test, since the variants share no discretization of the
regularizing term; and measure whether the expected robustness and cost advantages
of~(\ref{eq:Jpi_iso}) are realized in practice.
...
directly, as a function of normalized enclosed volume $s=V/V_\Omega\in[0,1]$, with no source and
no auxiliary parameters. The
...
Given the boundary shape, the toroidal flux $\Psi_t$, an edge pressure $p_b>0$, an
enclosed-toroidal-current profile $I_0(s)$ on $0\le s\le1$ with $I_0(0)=0$ and an edge value
$u_b$ of $u$, coefficients $\kappa_\parallel\gg\kappa_\perp>0$ and $D_u>0$, and \emph{either} a
heating deposition $S_p(\br)\ge0$ (predictive mode) \emph{or} a target
pressure-versus-normalized-volume profile $p_0(s)$, decreasing on $0\le s\le1$, with
$p_0(1)=p_b$ (interpretive mode), find $\bB(\br)$,
$p(\br)$, $u(\br)=G(s(\br))+\tilde u(\br)$ with the one-dimensional profile $G(s)$ [and, in
interpretive mode, $\chi(\br)$] on $\Omega$ satisfying
\begin{align}
\nabla\cdot\bB&=0,\qquad \mu_0\,\bJ=\nabla\times\bB ,
\tag{M1}\label{eq:M1}\\[2pt]
\bJ&=u\,\bB+\frac{\bB\times\nabla p}{B^{2}}-D_u\,\gradperp\tilde u ,
\qquad \tilde u\equiv u-G(s),
\tag{M2}\label{eq:M2}\\[2pt]
\bB\cdot\nabla u-\nabla\cdot\big(D_u\,\gradperp\tilde u\big)
&=\frac{2}{B^{3}}\,\bB\cdot\big(\nabla p\times\nabla B\big)
-\frac{\mu_0\,u}{B^{2}}\,\bB\cdot\nabla p
+\frac{\mu_0\,D_u}{B^{2}}\,\gradperp\tilde u\cdot\nabla p ,
\tag{M3}\label{eq:M3}\\[2pt]
\frac{1}{2\pi}\int_{s(\br')<s}\bJ\cdot\nabla\phi\;d^{3}r'&=I_0(s)\qquad(0<s\le1),
\tag{M3b}\label{eq:M3b}
\end{align}
where $s(\br)$ is the normalized-volume label of the isobars [$s=V_\chi(\chi)/V_\Omega$ in
interpretive mode, Eq.~(\ref{eq:s_label})], together with the pressure closure. \emph{Predictive mode:}
\begin{align}
\nabla\cdot\Big[\kappa_\parallel\,\bb\,(\bb\cdot\nabla p)
+\kappa_\perp\big(\nabla p-\bb\,(\bb\cdot\nabla p)\big)\Big]&=-\,S_p .
\tag{M4}\label{eq:M4}
...
p(\br)&=p_0\!\left(\frac{V_\chi(\chi(\br))}{V_\Omega}\right)=p_0\big(s(\br)\big),
\qquad
...
(M1)--(M2); $\tilde u=0$ with $G(1)=u_b$, i.e.\ $u=u_b$, for (M3) ($u_b=0$ for a wall in vacuum);
and $p=p_b$ for (M4) or $\chi=0$ for (M4a), all on $\partial\Omega$.
\medskip
\end{minipage}}
\end{center}
...
constrained constitutive stress current $\bJ_\pi=-D_u\gradperp\tilde u$ of
Eq.~(\ref{eq:Jpi_constrained}); (M3) is quasineutrality, $\nabla\cdot\bJ=0$, written out for the
current~(M2)---its last term,
$\tfrac{\mu_0 D_u}{B^{2}}\gradperp\tilde u\cdot\nabla p=-\tfrac{\mu_0}{B^{2}}\bJ_\pi\cdot\nabla p$,
is the current-viscosity contribution that makes this identity \emph{exact}, so that
$\nabla\cdot\bJ=0$ holds identically whenever (M3) is satisfied, which is what makes Amp\`ere's
law in (M1) integrable---and (M3) reduces to the classical magnetic differential
equation~(\ref{eq:MDE}) as $D_u\rightarrow0$ on a foliated field; (M3b) is the enclosed-current
constraint of Sec.~\ref{sec:net_current}, which fixes the one-dimensional profile $G(s)$ and
thereby the level-set mean of $u$ that the regularization in (M3) would otherwise select on its
own---the current-profile freedom of the classical problem, restored as an input. The pressure closure replaces the degenerate constraint
...
the user's profile $p_0(s)$ supplies their \emph{values}. The parallel component of the momentum
...
given value is well defined for any field topology. Prescribing $p_0(s)$---pressure as a function
of normalized enclosed volume---is also a classical device: it is precisely the form in which profiles enter
...
from $0$ to $\max\chi$; with $p_0$ decreasing,
$p=p_0(V_\chi(\chi)/V_\Omega)=p_0(s)$ is an increasing function of
...
$\chi$, the volume on which $p$ exceeds $p_0(s)$ is exactly $sV_\Omega$, for every $s\in[0,1]$:
the pressure-versus-normalized-enclosed-volume relation of the solution \emph{is} $p_0(s)$, identically, by
...
(The current profile is different in this respect: because the current viscosity acts on the mean
of $u$, the enclosed-current input requires the explicit constraint (M3b) and the one-dimensional
unknown $G(s)$ of Sec.~\ref{sec:net_current}; the level-set label $s$ used there is the same
$V_\chi(\chi)/V_\Omega$.)
\item \emph{The parallel gradient is automatically small.} Writing $p=g(\chi)$ with
$g(\chi)=p_0(V_\chi(\chi)/V_\Omega)$, the chain rule gives
...
$p_0(s)$ is realized spatially as a nearly stepped profile: flat across the flattened regions, with
...
$p=p_0(V(\psi)/V_\Omega)$: precisely the classical specification of pressure versus normalized
enclosed volume, with
...
logarithmic singularity and the composed $g(\chi)=p_0(V_\chi(\chi)/V_\Omega)$ acquires a mild kink; the mollified
...
profile $p_0(s)$ is met exactly, while the source that would sustain it is only approximately
...
\end{equation}
Parametrizing the level sets by normalized enclosed volume $s=V/V_\Omega$ and using the co-area formula
$\rho(\hat\chi)\equiv-dV_\chi/d\hat\chi=\oint_{\chi=\hat\chi}dA/|\nabla\chi|$, the balance takes
the one-dimensional conductance form
\begin{equation}
\frac{1}{V_\Omega^2}\frac{d}{ds}\Big[\mathcal G(s)\,\frac{dp}{ds}\Big]=-\,\bar S(s),
\qquad
\mathcal G(s)\equiv\bigg[\oint_{\chi=\hat\chi}\frac{\nabla\chi\cdot K\cdot\nabla\chi}{|\nabla\chi|}\,dA\bigg]
\bigg[\oint_{\chi=\hat\chi}\frac{dA}{|\nabla\chi|}\bigg] ,
\label{eq:conductance}
\end{equation}
with $\bar S(s)$ the level-set average of $S_p^{\rm eff}$ and the factors of $V_\Omega$ following
from $d/dV=V_\Omega^{-1}d/ds$. The conductance $\mathcal G(s)$ is a purely
...
$\mathcal G$ includes the parallel-flux contribution $\kappa_\parallel(\bb\cdot\nabla\chi)^{2}$, which on
...
as $\mathcal G(s)$ becoming very large across flattened regions.

Three diagnostics follow. (i)~The one-dimensional source required to sustain the prescribed
profile,
$\bar S_0(s)=-V_\Omega^{-2}\tfrac{d}{ds}[\mathcal G(s)\,p_0'(s)]$, should be positive where power is meant to be
...
flattening (a substantial $p_0'$ across an $s$ interval where $\mathcal G$ is enormous) reveal themselves
...
$S_{\rm ref}^{(m+1)}(\br)=\bar S_0^{(m)}\big(V_\chi^{(m)}(\chi^{(m)}(\br))/V_\Omega\big)$ (under-relaxed,
...
The physical inputs are the boundary shape (or coil currents, below), $\Psi_t$, $p_b$, the
enclosed-current profile $I_0(s)$ with the edge value $u_b$, the transport coefficients, and either
$S_p$ or $p_0(s)$; the two one-dimensional inputs $p_0$ and $I_0$ are functions of the same
independent variable, normalized enclosed volume $s\in[0,1]$, as in a classical code. In \emph{predictive} mode, $\kappa_\perp$
...
\emph{interpretive} mode, the profile $p_0(s)$ is the input, no source is specified, and the model
...
prescription---and the toroidally averaged enclosed
current $I_{\rm tor}(s)$ of Eq.~(\ref{eq:Itor}) equals $I_0(s)$ to solver tolerance by
construction, its residual toroidal variation $\Delta I_{\rm tor}(s)$ measuring the small leakage
of current across the isobars (Sec.~\ref{sec:net_current}). In the
...
$p=p_0(V(\psi)/V_\Omega)$; on a general foliated field they reduce to classical
...
(with $G=0$ there, so that $\tilde u=0$ too),
...
on the right is proportional to $\nabla p$ or to $\gradperp\tilde u$. The transition problem is
...
toroidal flux $\Psi_t$ for (M1)--(M2); $u=0$ (i.e.\ $\tilde u=0$ with $G(1)=u_b=0$) for (M3),
with the enclosed-current constraint (M3b) imposed on the whole domain; and $p=p_b$ for (M4), or
$\chi=0$ for (M4a). The free-boundary variant (Sec.~\ref{sec:Seff}, closing paragraphs) is unchanged:
...
effective cross-field transport of Sec.~\ref{sec:p_properties}, visible as the conductance $\mathcal G(s)$
...
\subsection{Interpretive mode: an exact smooth vacuum from a plateau in $p_0(s)$}
...
Let $s_p=V_p/V_\Omega<1$ label the intended plasma volume and prescribe a profile with a smooth plateau,
\begin{equation}
p_0(s)=p_b+\big(p_{\rm axis}-p_b\big)\,\Theta\!\Big(\frac{s_p-s}{\Delta s}\Big),
\qquad
\Theta\ \text{smooth},\quad \Theta(x)=0\ \text{for}\ x\le0,\quad
\Theta\rightarrow1\ \text{for}\ x\gtrsim1 ,
\label{eq:p0_plateau}
\end{equation}
i.e.\ $p_0$ strictly decreasing on $[0,s_p)$ with a $C^{k}$ (or $C^\infty$) roll-off of
normalized-volume width $\Delta s=\Delta V/V_\Omega$, and $p_0\equiv p_b$ for $s\ge s_p$.
Then, by (M4b), $p(\br)=p_b$ identically on the exterior region
$\{\br:\,V_\chi(\chi(\br))/V_\Omega\ge s_p\}$, and
$\nabla p=p_0'(s)\nabla s\equiv0$ there---not small, but zero. The construction of
...
$g(\chi)=p_0(V_\chi(\chi)/V_\Omega)$, Eq.~(\ref{eq:Seff}) gives $S_p^{\rm eff}\equiv0$ throughout the exterior
...
one-dimensional sink $\bar S_0(s)$ across the roll-off, and the total power
$\Gamma(0)$ crossing into it, characterize how demanding the prescribed edge is. A pathological
prescription (e.g.\ a steep roll-off placed across a strongly chaotic shell where $\mathcal G(s)$ is
...
\subsection{The current channel: a flat $I_0$ on the exterior and confinement of $\tilde u$}
\label{sec:edge_current}

The pressure plateau disposes of $\bJ_\perp$; the remaining question is the parallel current.
With $u=G(s)+\tilde u$ [Eq.~(\ref{eq:u_split})] and the enclosed-current constraint (M3b), the
recommendations are:
\begin{enumerate}
\item \emph{Prescribe $I_0(s)$ constant across the exterior plateau, with a smooth roll-off matched
to that of $p_0$, and take $u_b=0$.} The current-carrying region is then $s<s_p$; on the plateau
$s>s_p$ the shell constraints~(\ref{eq:shell_constraints}) read $\Delta I_j=0$, and the boundary
condition on $u$ is $u=0$ on the wall, as befits a wall in vacuum. Physically motivated profiles do
this automatically---the bootstrap current vanishes with the pressure gradient---and an ohmic
profile should be cut off with the same roll-off as $p_0$. A smooth roll-off ($I_0'\rightarrow0$
and preferably $I_0''\rightarrow0$ at $s_p$) means that $G(s)$ itself tends smoothly to zero at
the plasma edge; a step in $I_0'$ at $s_p$ is admissible but forces $G'$ to be large over the
roll-off width in order to hold the mean of $u$ against the transverse diffusion, and manufactures
a correspondingly localized multiplier current $D_uG'\gradperp s$ there.
\item \emph{The exterior $\tilde u$ then decays in thin layers, and its mean is pinned by the
constraint.} On the plateau, every term on the right of (M3) vanishes identically
($\nabla p\equiv0$; and $G'\bB\cdot\nabla s$ vanishes with $G'$), so $\tilde u$ satisfies the
homogeneous equation $\bB\cdot\nabla\tilde u=\nabla\cdot(D_u\gradperp\tilde u)$ with $\tilde u=0$
on the wall; the only exterior parallel current is what diffuses transversally out of the plasma
across the last isobar. For the harmonics with $k_\parallel\ne0$, balancing streaming against
transverse diffusion, the penetration depth on a field line of parallel wavenumber $k_\parallel$
is
\begin{equation}
...
suppressed by the smoothness of $\Theta$. The level-set \emph{mean} of $\tilde u$ is a different
matter: streaming does not damp it, and left to itself it would diffuse across the entire exterior
region rather than decay in a thin layer (this is precisely the mechanism of
Sec.~\ref{sec:net_current} by which the regularization sets the mean current). It is the exterior
shell constraints $\Delta I_j=0$ that pin it, through $G$, to carry no net toroidal current; with a
smooth roll-off the required $G$ is essentially zero on the plateau, and the constraint is a
safeguard rather than an active correction. The exterior current is thus zero up to thin, bounded,
weakly driven layers hugging the plasma boundary. We therefore recommend imposing (M3b) on the
whole domain, including the exterior shells, rather than restricting the support of $G$ to the
plasma and relying on the exterior mean to decay by itself.
...
\item \emph{Place the roll-off deliberately.} The normalized-volume width $\Delta s$ of the
$p_0$ roll-off (physical volume $V_\Omega\Delta s$) should correspond to a radial width
$w_{\rm edge}$ exceeding both the flattening
...
$u=0$ on $\partial\Omega$, with the enclosed-current profile $I_0(s)$ constant on the exterior
plateau and rolled off smoothly, in step with $p_0$, at the plasma edge, and the constraint (M3b)
imposed on all shells out to the wall.
...
current iterate [$s=V_\chi(\chi)/V_\Omega$ in interpretive mode; the analogous label built on $p$, or the
previous iterate's $\chi$, in predictive mode],
\begin{equation}
\kappa_{\perp}(s)=\kappa_\perp^{\rm pl}
+\big(\kappa_{\rm vac}-\kappa_\perp^{\rm pl}\big)\,
\Theta\!\Big(\frac{s-s_p}{\Delta s_\kappa}\Big),
\qquad
\kappa_{\parallel}(s)=\max\big[\kappa_\parallel^{\rm pl}\,,\;\kappa_\perp(s)\big] ,
\label{eq:edge_ramp}
\end{equation}
so that the operator is strongly anisotropic in the plasma and roll-off shell and isotropic with
large $\kappa_{\rm vac}$ in the exterior. Keying the coefficients to $s$ makes them
...
Eq.~(\ref{eq:layer_width}), and weakly driven, the exterior mean of $u$ being held at zero by the
constraint (M3b); hence $\bJ\rightarrow0$ and, through (M1),
...
$\Delta s\rightarrow0$, $\varepsilon_\kappa\rightarrow0$, and $D_u\rightarrow0$, the plateau
...
\begin{equation}
\int_\Omega\varphi\big(p(\br)\big)\,d^3r
=V_\Omega\int_0^1\varphi\big(p_0(s)\big)\,ds
...
$\hat\chi(s)$, and with it the composed profile $p(s)$, is nearly flat and perfectly tame.
Tabulate $V_\chi$ on level values $\hat\chi_j$ chosen equispaced in normalized
\emph{volume} $s$ (not in $\chi$),
...
$s^{(k)}=V_\chi^{(k)}(\chi^{(k)})/V_\Omega$ and
$p^{(k)}=p_0\big(s^{(k)}\big)$ by pointwise composition; the prescribed profile
...
the physical source.
\item Solve (M3) with the enclosed-current constraint (M3b), in the bordered
form~(\ref{eq:bordered}) built on Eq.~(\ref{eq:utilde_equation}) and the shell
constraints~(\ref{eq:shell_constraints}), for $\tilde u^{(k)}$ and the $N$ coefficients of
$G^{(k)}(s)$, and set $u^{(k)}=G^{(k)}(s^{(k)})+\tilde u^{(k)}$: a linear steady
advection--diffusion problem, first order along $\bB^{(k)}$ and elliptic transversally, bordered
by tens of one-dimensional unknowns and constraint rows that are handled by a Schur complement
(the fixed-point update of Sec.~\ref{sec:net_current} on $G$ is an acceptable prototype). The label
$s^{(k)}=V_\chi^{(k)}(\chi^{(k)})/V_\Omega$ is already available from step~1.
...
self-consistency: verify that the converged $p$ realizes $p_0(s)$ to quadrature accuracy via the
layer-cake moments~(\ref{eq:layercake}), that on nested-surface cases interpretive mode reproduces
the classical prescription $p=p_0(V(\psi)/V_\Omega)$, and that a predictive run driven by the recovered
$S_p^{\rm eff}$ of Eq.~(\ref{eq:Seff}) returns the interpretive solution; and (vii)~current-profile
self-consistency (Sec.~\ref{sec:net_current}): the two-profile unit test---with $\bB$, $p$, $D_u$
frozen, two very different $I_0(s)$ must return their respective enclosed currents, whereas the
unconstrained $u=F(p)+\tilde u$ formulation returns the same $u$ for any $F$---together with the
$D_u$ scan showing $\langle\tilde u\rangle_s\rightarrow0$ and the multiplier current
$D_uG'\gradperp s\rightarrow0$ while $I_{\rm tor}(s)$ stays fixed, and $\Delta I_{\rm tor}(s)
\rightarrow0$ as $\varepsilon_\kappa,\epsilon_J\rightarrow0$.
...
$\mathbf{x}=(\mathbf{A},\chi,\tilde u,\mathbf g)$, the pressure being \emph{eliminated}
through the explicit composition $p=p_0(V_\chi(\chi)/V_\Omega)$ wherever it appears in (M2)--(M3) and the
$N$ coefficients $\mathbf g$ of the current profile $G(s)$ carried as unknowns against the $N$
constraint residuals~(\ref{eq:shell_constraints}); in predictive mode
(M1)--(M4) for $\mathbf{x}=(\mathbf{A},p,\tilde u,\mathbf g)$---with $\bB=\nabla\times\mathbf{A}$ so that
...
+\frac{p_0'\big(s(\br)\big)}{V_\Omega}\,
\delta V_\chi\big(\chi(\br)\big),
\qquad
...
anisotropic operator at every iterate. The current profile, by contrast, cannot be an explicit map
(Sec.~\ref{sec:net_current}) and does add $N$ one-dimensional unknowns and $N$ constraint rows;
but $N$ is tens, the border of Eq.~(\ref{eq:bordered}) is dense only along level sets and low-rank
in the shell index---the same structure as the nonlocal part of
Eq.~(\ref{eq:composition_linearization})---and its response is near-diagonal with a positive
weight [Eq.~(\ref{eq:G_response})], so it costs a Schur complement of negligible size and
introduces no conditioning hazard.
...
anisotropic operators, of the nonlinear unit vector $\bb=\bB/|\bB|$, of the constrained current
closure (the dependence of $s$, $G(s)$, and the shell integrals~(\ref{eq:shell_constraints}) on
$\chi$ and $\mathbf A$), of the composition~(\ref{eq:composition_linearization}), and of
...
composition $p=p_0(V_\chi(\chi)/V_\Omega)$ confines $p$ to the range of the prescribed profile at every
...
$\tfrac{\mu_0 D_u}{B^{2}}\gradperp\tilde u\cdot\nabla p$ term, so that $\nabla\cdot\bJ=0$ holds
exactly, with the geometry supplying the Pfirsch--Schl\"uter drive on the right-hand side. The
resistive/thermal-force terms $R_{e\parallel}$ are what, in the full electron momentum equation,
fix the mean (ohmic$+$bootstrap) parallel current through the surface-averaged parallel Ohm's law;
that equation is \emph{not} part of the present model, which is why the mean current must be
imported as the input $I_0(s)$ and enforced through the profile $G(s)$ of
Sec.~\ref{sec:net_current}. The split $u=G(s)+\tilde u$ is thus the natural separation of Ohm's
law into its friction-driven mean, here prescribed, and its geometry-driven fluctuation, here
computed; and the constrained flux $-D_u\gradperp\tilde u$ is the statement that the electron
perpendicular viscosity smooths the parallel current toward the friction-driven profile rather than
toward zero.
...
than the customary $F$), and denote the enclosed toroidal current of Eq.~(\ref{eq:Itor}) by
$I_{\rm tor}$ throughout. Amp\`ere's law in (M1) then reads
\begin{equation}
\mu_0\bJ=\nabla I\times\nabla\phi-\Delta^*\psi\,\nabla\phi ,
\qquad
\Delta^*\psi\equiv R^{2}\,\nabla\cdot\!\Big(\frac{\nabla\psi}{R^{2}}\Big)
...
$D_u\,(I/RB^{2})\,\bB\cdot\nabla\tilde u$ of the current-viscosity flux $-D_u\gradperp\tilde u$
[with $\tilde u=u-G(s)$, Sec.~\ref{sec:net_current}]; no assumption that $p$, $I$, or $u$ is a flux
function has been made anywhere. The poloidal component of
Amp\`ere's law with (M2) gives, likewise exactly,
\begin{equation}
\nabla I=\mu_0\,u\,\nabla\psi-\frac{\mu_0\,I}{B^{2}}\,\nabla p
-\mu_0 D_u R\;\mathbf{e}_\phi\times\big(\gradperp\tilde u\big)_{\rm pol} ,
\label{eq:Igrad}
\end{equation}
...
$\chi$ and supplemented by the composition (M4b) in interpretive mode, and (\ref{eq:axi_M3})
supplemented by the enclosed-current constraint (M3b) for $G$]: three coupled quasilinear
elliptic problems in $(R,Z)$ for $(\psi,\;p\ \text{or}\ \chi,\;u)$, bordered by the
one-dimensional profile $G(s)$. The toroidal Amp\`ere law
...
composition (M4b) returns $p=p_0(V(\psi)/V_\Omega)$ exactly: the classical prescription of pressure versus
normalized enclosed volume [item 4 of Sec.~\ref{sec:transplant}; cf.\ Grad's ``adiabatic'' profiles
\cite{grad1975}].
...
whose flux-surface average is not fixed by (M3) at all: it is the mean-current mode of
Sec.~\ref{sec:net_current}, and (M3b) supplies it. Substituting Eq.~(\ref{eq:axi_u}) and
...
\end{equation}
the Grad--Shafranov equation, with $I=I(\psi)$; and the enclosed-current constraint, written for
the flux surfaces [$s=V(\psi)/V_\Omega$, so that $I_0$ becomes a function of $\psi$] and evaluated
with $\mu_0J_\phi=-\Delta^*\psi/R$, gives $dI_{\rm tor}/d\psi=\oint J_\phi\,dl/|\nabla\psi|$, i.e.
\begin{equation}
I\,\frac{dI}{d\psi}
=\frac{\mu_0}{\langle R^{-2}\rangle}\Big[\frac{2\pi}{V'(\psi)}\,\frac{dI_0}{d\psi}-\bar p'(\psi)\Big] ,
\label{eq:I_ODE}
\end{equation}
which replaces the free function $II'(\psi)$ of the classical problem by the standard algebraic
relation between $II'$, $p'$, and the prescribed toroidal current density profile [equivalently,
Eq.~(\ref{eq:axi_u}) averaged, $\mu_0\langle u\rangle=I'+\mu_0I\bar p'\langle B^{-2}\rangle$,
with $\langle u\rangle$ the profile $G$ that (M3b) selects]. The reduced system---GS plus the
one-dimensional transport equation plus the enclosed-current relation~(\ref{eq:I_ODE})---is
...
leading term of a single system of PDEs rather than as separately postulated modules. We stress
that without (M3b) the reduction would \emph{not} recover this freedom: at finite $D_u$ the
flux-surface average of Eq.~(\ref{eq:axi_M3}) is a radial diffusion equation for $\langle u\rangle$
whose only data are the boundary value and the metric correlations of
Eq.~(\ref{eq:levelset_current_identity}), so the unconstrained model would return a single,
relaxed current profile for every prescribed pressure.
...
one for current; in interpretive mode the pressure input $p_0(s)$ is exactly the classical
pressure-versus-normalized-enclosed-volume prescription. A user of a GS code would recognize every input and
every output.
...
(\ref{eq:axi_M4})---or the $\chi$ transplant---and the current function by (\ref{eq:axi_M3})
together with the enclosed-current constraint (M3b) for $I_0(s)$. The free functions of GS become
outputs (predictive mode) or are specified in the volume parametrization and realized by
construction (interpretive mode: $p_0$ through the transplant, $I_0$ through the constraint); the
...
either sources or the pair $\{p_0(s),I_0(s)\}$, selects one. A minor but telling instance: where
...
current-viscosity closure, and the constrained mean-current closure of
Sec.~\ref{sec:net_current} that a practical code needs in order to accept a current profile.
...
$\kappa_\perp$ is supplied by a turbulence model and $I_0(s)$ by an ohmic-plus-bootstrap closure; a kinetic
...
$-D_u\gradperp\tilde u$; regularized current continuity (M3), which bounds the
Pfirsch--Schl\"uter response in layers of width $\propto D_u^{1/3}$ and excludes current sheets,
supplemented by the enclosed-current constraint (M3b) that fixes the one-dimensional profile
$G(s)$ from which $\tilde u=u-G$ is measured---the mean parallel current, which the regularization
would otherwise select on its own and which the classical problem leaves as an input; and the anisotropic,
...
geometry, and a user-prescribed pressure-versus-normalized-volume profile $p_0(s)$ is transplanted onto them
by the composition $p=p_0(V_\chi(\chi)/V_\Omega)$---realizing the prescribed profile exactly, with no
