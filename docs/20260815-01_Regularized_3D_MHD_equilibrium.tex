...
toroidal current versus normalized enclosed volume $s=V/V_Omegain[0,1]$---the same independent
...
\label{eq:bordered}
\end{equation}
where $A$ is the advection--diffusion operator already needed for (M3), $P$ its action on the $N$
basis functions of $G$ (through $-G'\bB\cdot\nabla s$ and the boundary term), and $C_u$, $C_G$ the
shell integrals of the $\tilde u$ and $G$ contributions to $\bJ\cdot\nabla\phi$ [including the
$D_uG'\gradperp s\cdot\nabla\phi$ term]. Since $N$ is tens while $\tilde{\mathbf u}$ has millions of
