...
- [x] **5.3** Anderson with fallback — `DESIGN.md` §13.3
  ...
  scale-equivariant SVD-regularized least squares, a 1e5 condition gate, history restart, and an
  exact damped-Picard fallback. On the complete milestone-5.2 unstable map at damping 0.3, depths
...
- [x] **6.1** Poincaré sections and field-line tracing — `DESIGN.md` §19, §8.6 · note: §4.3
  ...
  **B** and an H(div) **B**; trace data saved and reloaded with a versioned record;
  ...
  6 and 48 tetrahedra cover (B_z,R₀)=(1,1) and (0.5,2.5), recover
  ...
  `PeriodicCylinder3D`, publish a solver-facing wrapper around the internal H(div)
