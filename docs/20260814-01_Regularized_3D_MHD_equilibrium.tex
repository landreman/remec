...
The constitutive relation~(\ref{eq:Jpi}) regularizes current continuity with a purely
transverse flux, $\bJ_\pi=-D_u\gradperp u$. A natural variant replaces the perpendicular
gradient by the full gradient,
\begin{equation}
\bJ_\pi=-\,D_u\,\nabla u ,
\label{eq:Jpi_iso}
\end{equation}
which adds a parallel component $-D_u\,\bb\,(\bb\cdot\nabla u)$ to the constitutive
current. Because a parallel current exerts no force, the perpendicular force
...
$O(D_u)$: $\bJ\cdot\bB/B^{2}=u-(D_u/B)\,\bb\cdot\nabla u$. One simply treats $u$ as the
auxiliary unknown and reconstructs $\bJ$ from~(\ref{eq:J_decomposition})
with~(\ref{eq:Jpi_iso}); diagnostics, and any closure consuming $J_\parallel$ (e.g.\ a
bootstrap model for $F$ in Sec.~\ref{sec:net_current}), must use the reconstructed
current. Second, exactness of $\nabla\cdot\bJ=0$ requires the correction term of
...
scalings~(\ref{eq:layer_width}) are unchanged. The extra terms generated in the $F(p)$
split of Sec.~\ref{sec:net_current} are $O(D_u\varepsilon_\kappa)$, negligible beside the
terms already kept in Eq.~(\ref{eq:utilde_equation}). Entropy production remains
sign-definite, $\int D_u|\nabla u|^{2}\ge\int D_u|\gradperp u|^{2}\ge0$. On a boundary
