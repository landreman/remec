...
with SciPy `solve_ivp`. DOP853 was selected because the live tolerance study below
reaches roundoff-level errors without event interpolation: the requested Poincare planes
are supplied directly as `t_eval`. The same public tracer accepts an ordinary analytic
...
field `B=(-0.37*y, 0.37*x, 1)` on 6- and 48-tetrahedron meshes produces iota errors
1.34e-15 and 3.34e-16, radial excursions below 3.67e-15, and independently measured
divergence-squared integrals below 1.03e-28.
...
short live check restarts from the accepted row-6 state; a separate reachability run
...
The reconstructed (M2) shell residual is a same-mapped-quadrature diagnostic, not an
independent physics oracle. A separate order-8 domain integral pins its nonzero total
to `0.60631864`, while the live row pins protect the spatial solution.
...
and Cerfon--Freidberg (2010), Eqs. (5)--(12). It solves the papers' coefficient
constraints, evaluates \(\psi\), its first and second derivatives, extracts the
\(\psi=0\) boundary, and evaluates the current, beta, volume, perimeter, and flux
integrals. The Cerfon--Freidberg ITER-like smooth case gives
\(\beta_t=0.04913\) for \(q^*=1.57\), consistent with the paper's quoted 0.05.
The Zheng Figure-1 solve realizes \(I_p=1\,\mathrm{MA}\) and
\(\beta_{pol}=0.4\) to relative error below \(2\times10^{-10}\).

The shaped domain is made directly from the analytic constant-flux contour, so the
existing homogeneous-Dirichlet weak form applies without a rectangular boundary lift:

\[
\int_\Omega \frac{\nabla\psi_h\cdot\nabla v}{R}\,dR\,dZ
=\int_\Omega\left(\mu_0R p'(\psi)+\frac{II'(\psi)}{R}\right)v\,dR\,dZ.
\]

Quadratic CAD splines are sampled from the exact contour and the mesh is curved to
degree \(p+1\). The boundary error below is the RMS analytic flux on the numerical
wall, normalized by the analytic axis flux. Rates use the unstructured two-dimensional
scale \(h_{eff}\propto n_e^{-1/2}\), not the requested mesher `maxh`. The complete
table is `tests/verification/shaped_zheng_rates.csv` and is regenerated by
`scripts/generate_shaped_grad_shafranov_tables.py`.

| Degree \(p\) | Elements (coarse → fine) | finest-pair L² rate | finest-pair weighted-energy rate | finest boundary error |
| ---: | ---: | ---: | ---: | ---: |
| 1 | 424 → 1019 | 1.894 | 0.930 | 2.681e-7 |
| 2 | 424 → 1019 | 3.073 | 1.977 | 2.644e-7 |
| 3 | 424 → 1019 | 4.394 | 3.149 | 2.639e-7 |

The not-slow sentinel uses \(p=2\) on two meshes and enforces the same
\(p+0.8\)/\(p-0.2\) gates. Axis radius comes from the computed midplane extremum. The
four shape observables come from ray intersections with the computed interior level
set \(\Psi_h=0.05\Psi_{h,axis}\) and are compared with the corresponding analytic
level set, not with the input boundary. On the finest sentinel mesh, the major/minor
radius errors are 1.91e-6/4.98e-6, the elongation error is 1.68e-5, and the
triangularity error is 4.14e-6. A regression mutation changes the homogeneous flux
shape and must move the recovered major radius and triangularity; identically zero
flux is rejected.

The double-null Cerfon--Freidberg case retains both X-point corners as distinct CAD
vertices and uses analytic per-arc parameterizations rather than a fixed sampled
polygon. On 126 → 201 → 358 → 706 → 1537 elements its relative L² error falls
3.088e-3 → 1.152e-3 → 4.799e-4 → 1.804e-4 → 4.670e-5, and the absolute
boundary-flux geometry error falls 3.388e-4 → 6.340e-5 → 1.950e-5 → 4.124e-6 →
1.432e-6. The finest-pair effective rates are 3.473 and 2.719, with no fixed-polygon
plateau. All free-DOF algebraic residuals are below 6.73e-15. These measurements are in
`tests/verification/cerfon_freidberg_xpoint_rates.csv`; the X-point scan and the full
smooth order scan are marked `slow` and remain far below the 90-second per-test budget.

Mutation checks remove the independent \(II'\) Solov'ev source and incorrectly treat
the analytic curve midpoint as Netgen's quadratic control point. Both changes make the analytic
benchmark fail while the sparse solve still converges algebraically, distinguishing
physics/geometry accuracy from residual convergence. A third regression perturbs a
homogeneous computed flux while holding the mesh fixed and confirms that the interior
level-set shape observables respond.
...
at least \(1/(1+10^{-2})\). Configuration rejects any custom pair with
\(\lambda\kappa_{\max}^2>10^{-2}\), so this accuracy bound cannot be bypassed by changing only
one knob. A deficient or overly conditioned history is cleared, the current pair is retained as
the new history origin, and that cycle accepts exactly \(x_k+\beta f_k\).
...
after substituting \(V'(\psi)=V_\Omega ds/d\psi\), so the two factors of
\(ds/d\psi\) cancel only in its current-profile term:
...
\(I_0=0.4s(2-s)\), including both edge totals. The verification uses
\(\mu_0=2.3\) and the nonconstant map derivative \(ds/d\psi=0.6+0.5s\).
...
\(4.52\), despite their target arrays remaining well formed. After review, deleting
the separate \(\mu_0Rp'(\psi)\) GS source raises the degree-2 coarse L² error to
\(8.59\times10^{-2}\); forcing the closure's \(\mu_0\) to one makes the two profile
errors \(2.57\) and \(2.40\); and replacing the chain-rule product by division fails
the nonconstant-\(ds/d\psi\) derivative check by as much as \(1.78\).
...
\[
\mu_0^{-1}(\nabla\times A_h,\nabla\times v_h)
+(\nabla\lambda_h,v_h)=(J,v_h),\qquad
(A_h,\nabla q_h)=0,
\]

with essential \(n\times A_h=0\) and \(\lambda_h=0\) on the complete boundary. The
spaces use the milestone-4.1 tetrahedral pairing
\(A_h\in H(\mathrm{curl})_p\), \(\lambda_h\in H^1_{p+1}\), and the reported
field is an independent mass projection of \(\nabla\times A_h\) into
\(H(\mathrm{div})_{p-1}\). No mass penalty is added to the vector equation.

On the unit cube the manufactured potential
\(A=(0,0,\sin(\pi x)\sin(\pi y))\) is divergence-free, has zero tangential trace,
and gives
\(J=(0,0,2\pi^2\sin(\pi x)\sin(\pi y))/\mu_0\). The 2, 3, and 4 cells-per-axis
sweep measures the following finest-pair rates:

| HCurl base p | A L² rate | B=curl(A) L² rate | finest A error | finest B error |
| ---: | ---: | ---: | ---: | ---: |
| 1 | 1.8593 | 0.9414 | 5.5283e-2 | 6.7660e-1 |
| 2 | 3.0036 | 1.9306 | 3.3434e-3 | 9.9018e-2 |
...
\(\lambda_h=\phi_h\) to 1e-11, so the gauge block is tested independently of the
