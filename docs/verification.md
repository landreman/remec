...
\(p+0.8\)/\(p-0.2\) gates. Axis radius and all four boundary-shape observables are
recovered from \(\Psi_h\) and the numerical boundary rather than copied from analytic
coefficients. The finest sentinel errors are below \(2\times10^{-3}\) in major/minor
radius, \(10^{-2}\) in elongation, and \(2\times10^{-2}\) in triangularity.

The double-null Cerfon--Freidberg case retains both X-point corners as distinct CAD
vertices. On 93 → 179 → 340 elements its relative L² error falls
4.629e-3 → 1.575e-3 → 5.220e-4, and the absolute boundary-flux geometry error falls
1.662e-4 → 6.401e-5 → 2.443e-5. The finest-pair effective rates are 3.442 and 3.003.
All free-DOF algebraic residuals are below 1.55e-15. These measurements are in
...
physics/geometry accuracy from residual convergence.
