...
\(5.10\times10^{-6}\), \(1.22\times10^{-5}\), and \(6.45\times10^{-5}\).
The fast PR test samples the degree-3, 128-element slice and keeps the relative L²
error below \(5.6\times10^{-4}\) throughout the anisotropy axis.
...
positive recorded floor activity, and a free-DOF relative residual below \(10^{-10}\).
The 27-case Cartesian scan is marked `slow`; `.github/workflows/nightly.yml` runs
`make test-full` every day and on manual dispatch, while PR CI keeps the smaller
finite-anisotropy slice plus all nine island convergence solves.
