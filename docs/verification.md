...
difference from physical \(\kappa_\perp\), so its redundant table pin uses relative
tolerance \(10^{-2}\), consistent with the measured quadrature sensitivity; the strict
order/refinement assertions remain the acceptance test for that derived metric.
...
sense. STATUS assigns that separate sensitivity study to milestone 1.5. The rate table is
