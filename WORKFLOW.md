...
**CI:** runs `make check` on Linux and macOS. `nightly.yml` runs the bounded developer
suite on the oldest and newest Python versions; `exhaustive.yml` runs the
remote-exhaustive suite once on a canonical numerical environment. A milestone agent
dispatches `exhaustive.yml` on its branch only when it changes exhaustive behavior, then
continues other work while it runs; a complete green run is required before the milestone
PR is review-ready.
...
developer suite, `codex exec` in a loop. Prefer the manually dispatched `exhaustive.yml`
workflow for `make test-exhaustive`; it lets the large 3D ladders run remotely while
